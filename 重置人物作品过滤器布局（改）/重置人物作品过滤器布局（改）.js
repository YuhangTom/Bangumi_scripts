...
(function () {
  ...
  // ===== ONLY FIX START =====
  // Wait until Bangumi finishes rendering
  function waitInit(retry = 0) {
    ...
    if (!subjectFilter) {
      ...
      if (retry < 30) {
        setTimeout(() => waitInit(retry + 1), 300);
      }
    ...
    if (subjectFilter.dataset.relayoutDone) return;
    subjectFilter.dataset.relayoutDone = '1';
